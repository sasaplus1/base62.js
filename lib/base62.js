...
(function(global) {
  ...
  function Base62(table) {
    ...
    if (arguments.length <= 0) {
      ...
      throw new TypeError('parameter must be a string: ' + tableType);
  ...
  Base62.prototype.decode = function(str) {
    ...
    // type check
    if (strType !== 'string') {
      throw new TypeError('parameter must be a string: ' + strType);
    }
  ...
  Base62.prototype.encode = function(num) {
    ...
    // type check
    if (numType !== 'number') {
      throw new TypeError('parameter must be a number: ' + numType);
    }
